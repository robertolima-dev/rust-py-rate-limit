...
//!
//! This is a thin `#[pyclass]` wrapper around [`FixedWindow`] plus the activity
//! counters. The heavy lifting (the per-key map operations) runs inside
//! `py.allow_threads(...)`, which releases the GIL so multiple Python threads
//! can hit the limiter concurrently and exercise DashMap's per-shard locking.
//!
...
impl RateLimiter {
    ...
    ///
    /// Both arguments must be positive; `0` (or a negative value, which fails
    /// the `u64` conversion with `OverflowError`) raises `ValueError`.
    #[new]
    #[pyo3(signature = (limit, window_seconds))]
    fn new(limit: u64, window_seconds: u64) -> PyResult<Self> {
        if limit == 0 {
        ...
        Ok(Self {
            inner: FixedWindow::new(limit, window_seconds),
    ...
    fn __repr__(&self) -> String {
        format!(
            "RateLimiter(limit={}, window_seconds={})",
            self.inner.limit, self.inner.window_seconds
        )
    }
}
